...
for zip_dirty in $(find dist -regex ".*_dirty\.\(zip\)");do
  ...
  curl -SL "http://download.newrelic.com/infrastructure_agent/binaries/linux/noarch/nrjmx_linux_${NRJMX_VERSION}_noarch.tar.gz" | tar xz; cp usr/bin/nrjmx.jar "${JMX_IN_ZIP_PATH}/nrjmx.jar"
